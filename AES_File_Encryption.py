...
class MainWindow:
    ...
    def __init__(self, root):
        ...
        try:
            icon_img = tk.Image(
                "photo", file=self.THIS_FOLDER_G + "./files/logo.ico"
            )
        ...
        self.key_entry_label1 = tk.Label(
            root,
            text="Enter Key (Remembered For Decryption)",
            bg="#eeeeee",
            anchor=tk.W,
        )
        ...
        self.key_entry_label2 = tk.Label(
            root, text="Re-enter Key", bg="#eeeeee", anchor=tk.W
        )
    ...
    def unfreeze_controls(self):
        ...
        self.decrypt_btn.configure(state="normal", bg="#a1161b")
    ...
    def e_check_callback(self):
        ...
        if newPath.is_file():
            ...
            messagebox.showinfo("AES File Encryption", "Please Enter a valid File URL !!")
            return

        if len(self._secret_key.get()) == 0:
            messagebox.showinfo("AES File Encryption", "Please Enter a valid Secret Key !!")
            return
        elif self._secret_key.get() != self._secret_key_check.get():
            messagebox.showinfo("AES File Encryption", "Passwords do not match !!")
            return
...
Please Enter an Encrypted File to Decrypt.""",
        ...
        if len(self._secret_key.get()) == 0:
            messagebox.showinfo("AES File Encryption", "Please Enter a Secret Key !!")
            return
        elif self._secret_key.get() != self._secret_key_check.get():
            messagebox.showinfo("AES File Encryption", "Passwords do not match !!")
            return
    ...
    def encrypt_execute(self):
        ...
        try:
            ...
            if self.should_cancel:
                ...
                messagebox.showinfo("AES File Encryption", "Cancellation Successful !!")
                self._cipher = None
                self.should_cancel = False
            ...
            messagebox.showinfo("AES File Encryption", "File Encryption Successful !!")
        except Exception as e:
    ...
    def decrypt_execute(self):
        ...
        try:
            ...
            messagebox.showinfo("AES File Encryption", "File Decryption Successful !!")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    path_to_ico = "files/logo.ico"  # Replace this with the actual path to logo.ico
    
    # Check if the ICO file exists
    if Path(path_to_ico).is_file():
        # Set the icon for the Tkinter root window
        ROOT.iconbitmap(path_to_ico)
    else:
        print("ICO file not found:", path_to_ico)
